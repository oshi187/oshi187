# 👋 Hi, I'm Oshini

🎓 Computer Engineering Student at HAW Hamburg (TI-ETECH)
💻 Interested in Embedded Systems, C Programming, and Software Development

---

## 🚀 Projects

### 🔌 Microcontrollers

* GPIO & 7-Segment Display
* Ultrasonic Distance Measurement
* Interrupt-Based Systems (NVIC)
* UART Communication
* Final Embedded Project

### 📘 Software Construction

* C Programming (Algorithms, Math, Mandelbrot)
* Object-Oriented Programming in Java
* GUI Applications & Recursion

### 📊 Algorithms & Data Structures

* Sorting Algorithms (Quick, Merge, Bubble, etc.)
* Hash Tables (Address & Chained Hashing)
* Queues & Lists

---

## 🛠️ Skills

* **Languages:** C, Java
* **Concepts:** OOP, Data Structures, Algorithms
* **Embedded:** GPIO, Timers, Interrupts, UART
* **Tools:** Code Composer Studio, GCC, Git

---

## 📫 Contact

* 📧 **Email:** oshinividj@gmail.com
* 💼 **LinkedIn:** https://www.linkedin.com/in/oshini-jayaweera/
* 💻 **GitHub:** https://github.com/oshi187

---

## 🌱 Currently Learning

* Advanced Embedded Systems
* Real-Time Programming
* Software Design & Architecture

---

⭐ Feel free to explore my repositories!
